...
export default function LivePage() {
  ...
  const polling =
    (status?.state === 'running' || status?.state === 'queued') && !websocketConnected
  ...
  // ── when snapshot becomes available via status flag ──────────────
  useEffect(() => {
    if (!sessionId) return
    if (status?.has_snapshot && !snapshot?.symbols) {
      dispatch(fetchLiveSnapshot(sessionId))
    }
  }, [dispatch, sessionId, status?.has_snapshot, snapshot?.symbols])
  ...
  useKeyboardShortcuts({
    shortcuts: [
      { key: 'b', action: () => { dispatch(resetLive()); navigate('/') } },
      { key: 's', action: handleStop },
    ],
  })
  ...
  const dotClass =
    st === 'running' ? 'running' : st === 'error' ? 'failed' : st === 'stopped' ? 'finished' : ''
  const statusText = status?.error ? `${st} (${status.error})` : st

  const canStop = Boolean(sessionId) && (st === 'running' || st === 'queued')
  ...
  const paramsEntries = useMemo(() => {
    ...
    if (p.backtest_args && Object.keys(p.backtest_args).length)
      sections.push({ title: 'Live', entries: Object.entries(p.backtest_args) })
    if (p.env_overrides && Object.keys(p.env_overrides).length)
      sections.push({ title: 'Env overrides', entries: Object.entries(p.env_overrides) })
    if (p.meta && Object.keys(p.meta).length)
      sections.push({ title: 'Meta', entries: Object.entries(p.meta) })
    return sections
  }, [status?.params])
  ...
  return (
      ...
      title={
        <div className="row">
          ...
          <span
            className="pill"
            style={{
              backgroundColor: websocketConnected ? '#4CAF50' : '#FF9800',
              color: 'white',
              fontSize: '0.8em',
              marginLeft: 8,
            }}
          >
            {websocketConnected ? 'WS' : 'Poll'}
          </span>
        </div>
      }
    ...
    >
              ...
              <div key={idx} style={{ display: 'grid', gap: 10 }}>
                {col.map(([k, v]) => (
                  <div key={k} className="pill">
                    <b>{k}</b>
                    <span className="muted">{safeString(v)}</span>
                  </div>
                ))}
              </div>
            )).flatMap((node, idx) =>
              idx === 0 ? [node] : [<div key={`sep-${idx}`} style={{ width: 1, background: 'rgba(255,255,255,0.12)' }} />, node]
            )}
          </div>
        ) : (
      ...
      {/* ── Charts (mirrors BacktestPage Status card) ────────── */}
      <Card
        title={
          <div className="row">
        ...
        {flatParams.length ? (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr 1px 1fr', gap: 14, alignItems: 'stretch' }}>
            {paramCols.map((col, idx) => (
              ...
              idx === 0 ? [node] : [<div key={`psep-${idx}`} style={{ width: 1, background: 'rgba(255,255,255,0.12)' }} />, node]
            )}
          </div>
        ) : (
          <div className="muted">No parameters found for this session.</div>
        )}
      </Card>
    </Layout>
  )
